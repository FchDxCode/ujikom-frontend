...
import 'package:gallery_fe/presentation/screens/ManageUser_screens.dart';
...
class NavbaradminScreens extends StatefulWidget {
  const NavbaradminScreens({super.key});

  @override
  _NavbarAdminScreen createState() => _NavbarAdminScreen();
}

class _NavbarAdminScreen extends State<NavbaradminScreens> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  Widget _currentBody = const Center(child: Text("Selamat Datang Admin"));
  String _currentRoute = '/dashboard';

  @override
  void initState() {
    super.initState();
    _updateBody('/dashboard');
  }
    ...
    setState(() {
      ...
      switch (route) {
        case '/dashboard':
          ...
          break;
        case '/analytics':
          _currentBody = const AnalyticsScreen();
          break;
        case '/manageUsers':
          _currentBody = const ManageUsersScreen();
          break;
        case '/managePages':
          _currentBody = const ManagePagesScreen();
          break;
        case '/manageContentBlock':
          _currentBody = const ManageContentblockScreen();
          break;
      }
    });
  }
    ...
    return BlocListener<AuthBloc, AuthState>(
      ...
      child: Scaffold(
        ...
        appBar: isLargeScreen
            ? null
            : AppBar(
                title: Text('Admin Panel', style: GoogleFonts.poppins(color: AppColors.stoneground)),
  ...
  Widget _buildNavMenu() {
    final List<Map<String, dynamic>> menuItems = [
      ...
      {'title': 'Manajemen Users', 'icon': Icons.people, 'route': '/manageUsers'},
      {'title': 'Manajemen Pages', 'icon': Icons.pages, 'route': '/managePages'},
      {'title': 'Content Blocks', 'icon': Icons.view_module, 'route': '/manageContentBlock'},
    ];
      ...
      children: [
          ...
          child: Column(
            ...
            children: [
              const CircleAvatar(
                ...
                child: Icon(Icons.admin_panel_settings, size: 40, color: AppColors.shadow),
              ),
              const SizedBox(height: 16),
              Text(
                'Admin Panel',
                style: GoogleFonts.poppins(color: AppColors.stoneground, fontSize: 20, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ),
        ...menuItems.map((item) {
          ...
          return ListTile(
            leading: Stack(
              ...
              children: [
                Icon(item['icon'], color: isSelected ? Colors.blue : AppColors.cerramic),
              ],
            ),
            title: Text(
              item['title'],
        ...
        ListTile(
          ...
          onTap: () {
            ...
            ProfileIcon.onSelected(Menu.signOut, context);
          },
        ),
      ],
    );
